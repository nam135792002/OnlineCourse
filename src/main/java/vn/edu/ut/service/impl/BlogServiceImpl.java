...
import vn.edu.ut.repository.BlogRepository;
import vn.edu.ut.repository.UserRepository;
import vn.edu.ut.service.BlogService;
...
public class BlogServiceImpl implements BlogService {
    ...
    public BlogResponse save(BlogRequest blogRequest, MultipartFile img) {
        ...
        if(blogRepository.existsBlogByTitle(blogRequest.getTitle())){
            throw new AppException(HttpStatus.BAD_REQUEST, "Tên blog này đã tồn tại!");
        }
        ...
        if(blogRepository.existsBlogBySlug(slug)){
            throw new AppException(HttpStatus.BAD_REQUEST, "Vui lòng thay đổi tên blog: Tên Slug bị trùng");
        }
        blog.setSlug(slug);
    ...
    public BlogResponse update(Integer blogId, BlogRequest blogRequest, MultipartFile img) {
        ...
        if(!Objects.equals(blogRequest.getUserId(), blogInDB.getUser().getId())){
            throw new AppException(HttpStatus.FORBIDDEN, "Bạn không phải tác giả bài viết nên không thể truy cập tính năng này!");
        }
        ...
        if(checkBlogDuplicate != null){
            if(!(Objects.equals(blogInDB.getId(), checkBlogDuplicate.getId()))){
                throw new AppException(HttpStatus.BAD_REQUEST, "Tên blog/slug đã tồn tại trước đây");
            }
        }
